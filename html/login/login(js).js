    ...
    constructor: EmailAutoComplete, 
     init: function(options){
        ...
        $(_config.targetCls).each(function(index,item){
			...
			$(index).addEvent('keyup',function(e){
                var target = e.target,
                    //targetVal = $.trim($(this).val()),
                    targetVal = this.value.trim();
                    keycode = e.keyCode,
                ...
                if(targetVal == '') {
                    ...
                    $(index).attr({'data-html':targetVal}); 
                     // 给隐藏域赋值
          ...
          var target = this,
           ...
           if(!$(target).hasClass(tagCls)) {
             $('.auto-tip') && $('.auto-tip').each(function(index,item){
                 !$(index).hasClass('hidden') && $(index).addClass('hidden');
             });
          }
       });
    },
            ...
            if(_cache.onlyFlag) {
                ...
                var wrap = '<ul class="auto-tip">'; 
                 for(var i = 0; i < _config.mailArr.length; i++) { 
                     wrap += '<li class="p-item'+i+'">'+'<span class="output-num"></span><em class="em" data-html="'+_config.mailArr[i]+'">'+_config.mailArr[i]+'</em></li>';
                }
                wrap += '</ul>';
                ...
                $('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.height,'width':cfg.width - 2 + 'px','left':0,
                    'border':'1px solid #ccc','z-item':10000});
            }

            // 给所有li添加属性 data-html
            $('.auto-tip li',cfg.parentNode).each(function(index,item){
                $('.output-num',index).html(curVal);
                !$('.output-num',index).hasClass(_config.inputValColor) && 
                $('.output-num',index).addClass(_config.inputValColor);
                var emVal = $.trim($('.em',index).attr('data-html'));
                $(index).attr({'data-html':curVal + '' +emVal});
            }); 
             // 精确匹配内容
        ...
        if(/@/.test(curVal)) {
            ...
            if(newArrs.length > 0) {
                ...
                for(var j = 0, jlen = newArrs.length; j < jlen; j++) {
                    html += '<li class="p-item'+j+'">'+'<span class="output-num"></span><em class="em" data-html="'+newArrs[j]+'">'+newArrs[j]+'</em></li>';
                }
                $('.auto-tip',cfg.parentNode).html(html);

                // 给所有li添加属性 data-html
                $('.auto-tip li',cfg.parentNode).each(function(index,item){
                    $('.output-num',index).html(prefix);
                    !$('.output-num',index).hasClass(_config.inputValColor) && 
                    $('.output-num',index).addClass(_config.inputValColor); 
                     var emVal = $.trim($('.em',index).attr('data-html'));

                    $(index).attr('data-html','');
                    $(index).attr({'data-html':prefix + '' +emVal});
                }); 
    ...
    _removeBg: function(parentNode){
        var self = this,
         ...
         $(".auto-tip li",parentNode).each(function(index,item){
            $(index).hasClass(_config.hoverBg) && $(index).removeClass(_config.hoverBg);
        });    
    },
    /**
     * 键盘上下键操作
        ...
        if($('.auto-tip' + ' li',parentNode) && $('.auto-tip' + ' li').length > 0) { 
            ...
            if(keyCode == 38) {
                ...
                if(_cache.currentIndex !== -1) {
                     ...
                     !$('.auto-tip .p-item'+_cache.currentIndex,parentNode).hasClass(_config.hoverBg) &&
                    $('.auto-tip .p-item'+_cache.currentIndex,parentNode).addClass(_config.hoverBg).siblings().removeClass(_config.hoverBg); 
                     var curAttr = $('.auto-tip' + ' .p-item'+_cache.currentIndex,parentNode).attr('data-html');
                ...
                var curVal = $('.auto-tip' + ' .p-item'+_cache.oldIndex,parentNode).attr('data-html');
